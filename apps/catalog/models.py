...
class Category(MPTTModel, MetaTagMixin):
    ...
    def image_tag_thumbnail(self):
        if self.image:
            return mark_safe(f"<img.scr='/{MEDIA_ROOT}{self.image}' width='70'/>")

    image_tag_thumbnail.short_description = 'Изображение'

    def image_tag(self):
        if self.image:
            return mark_safe(f"<img.scr='/{MEDIA_ROOT}{self.image}'/>")
...
class ProductImage(models.Model):
    ...
    is_main = models.BooleanField(verbose_name='Основное изображениеб', default=False)
